...
This analysis considers only WSL data. 
...
wstd%>% 
  ...
  group_by(Name, rec_num, system) %>%
        summarize("No Blank" = mean(norm_ratio), 
                  ...
                  "Wh WS LBC" = mean(FmLBCWh, na.rm = TRUE),
                  "Wh WS LBC err" = mean(FmLBCerrWh, na.rm = TRUE),
...
```{r}
wstd%>%
  filter(!is.na(FmLBCWh)) %>%
  pivot_longer(c(fm_corr, FmLBC, FmLBCWh), names_to = "Method", values_to = "Fm") %>%
...
base + 
  ...
  stat_function(fun = fun, args = list(FmB = 0.004), color = "orange") +
  stat_function(fun = fun, args = list(FmB = 0.001), color = "green") +
