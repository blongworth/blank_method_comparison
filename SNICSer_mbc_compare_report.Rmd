...
This report compares data for a suite of wheels analysed with the production
version of SNICSer (2.84) with results from the same wheels analysed with a new
version of SNICSer that includes the following changes to the blank correction
method:
...
The same flagging and large wheel blank values are used for both runs. The data
for the test version was read from the raw results files, and the flagging from
the production run was read in from a saved file. This is the method used:
...
The normalization method and large blank values were set to match the origninal analysis. norm_ratio, int_err, ext_err, lg_blk_fm, and sig_lg_blk_fm were checked and are the same for both analyses. 
...
Using large blanks from the wheel rather than stored values leads to discrepancies in fm_corr of up to 0.0028 for small samples.
...
The differences in final fm for large samples not including -99s.
...
This section compares the new SNICSer calculations to what should be the same mass balance calculations done outside of SNICSer using Rand Excel (provided by Mark). Mark and I have both seen small differences before. The cause of the differences is uncertain, but they are small and do not significantly affect results.
